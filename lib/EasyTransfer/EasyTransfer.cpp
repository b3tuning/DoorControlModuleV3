...
boolean EasyTransfer::receiveData() {
	...
	// start off by looking for the header bytes.
	// If they were already found in a previous call, skip it.
	if(rx_len == 0) {
		// this size check may be redundant due to the size check below,
		// but for now I'll leave it the way it is.
		if(_stream->available() >= 3) {
			//this will block until a 0x06 is found or buffer size becomes less then 3.
			while(_stream->read() != 0x06) {
				// This will trash any preamble junk in the serial buffer
				// but we need to make sure there is enough in the buffer
				// to process while we trash the rest
				// if the buffer becomes too empty, we will escape and try again on the
				// next call
				if(_stream->available() < 3) {
					return false;
				}
	...
	// the struct size matched what we know, and now we are byte aligned.
	if(rx_len != 0) {
